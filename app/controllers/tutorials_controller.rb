class TutorialsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @tutorial.update(tutorial_params)
        format.html { redirect_to @tutorial, notice: "Tutorial was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to tutorials_path, notice: "Tutorial was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_tutorial
      @tutorial = Tutorial.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def tutorial_params
      params.expect(tutorial: [ :title, :slug, :content, :image ])
    end
end
